...
const CardButton: React.FC<{ onClick: () => void, children: React.ReactNode, className?: string, disabled?: boolean }> = ({ onClick, children, className = 'bg-indigo-600 hover:bg-indigo-700', disabled = false }) => (
    <button
        onClick={onClick}
        disabled={disabled}
        className={`px-4 py-2 text-sm font-semibold text-white rounded-md transition-colors duration-200 flex items-center justify-center ${className} ${disabled ? 'bg-gray-500 cursor-not-allowed opacity-70' : ''}`}
    >
        {children}
    </button>
);
...
const SopChooserCard: React.FC<{ onAction: CardRendererProps['onAction'] }> = ({ onAction }) => {
    ...
    return (
        <div>
            <h3 className="font-bold text-lg text-white flex items-center"><BookOpenIcon /><span className="ml-2">Choose a Guided SOP</span></h3>
            <p className="text-gray-300 mt-1">Select a workflow based on your role and objective.</p>
                        ...
                        <div className="mt-2 space-y-2 border-l-2 border-gray-700 pl-4">
                            {category.sops.map(sop => (
                                ...
                                >
                                    ...
                                    <p className="text-sm text-gray-400 mt-1">{sop.description}</p>
                                </button>
                            ))}
                        </div>
                    </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3">
            ...
            >
                <div className="flex items-center">
                    <TemplateIcon />
                    <p className="ml-2 font-semibold text-white">Start from a Project Template</p>
                </div>
...
const TemplateSelectorCard: React.FC<{ payload: any, onAction: CardRendererProps['onAction'], allTemplates: ConfigTemplate[], messageId: number }> = ({ payload, onAction, allTemplates, messageId }) => {
    return (
        <div>
            <h3 className="font-bold text-lg text-white flex items-center"><TemplateIcon /><span className="ml-2">Select a Template</span></h3>
            <p className="text-gray-300 mt-1">Choose a template to base your new configuration on.</p>
        ...
        <div>
            ...
            <div className="relative mt-3">
                ...
                <input
                    ...
                    className="w-full bg-gray-900 text-white placeholder-gray-400 border border-gray-600 rounded-md py-2 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
            </div>
            <div className="mt-3 max-h-60 overflow-y-auto space-y-2 pr-2">
                {filteredConfigs.map(config => (
                    <button 
                        ...
                        className="w-full p-3 bg-gray-700/50 hover:bg-indigo-900/50 border border-gray-600 hover:border-indigo-500 rounded-lg text-left transition-all"
                    >
                        <div className="font-semibold text-white">{config.projectName} <span className={`text-xs font-bold ${config.level === 'Project' ? 'text-indigo-400' : 'text-teal-400'}`}>({config.level})</span></div>
...
const ConfigWizardCard: React.FC<{ payload: any, onAction: CardRendererProps['onAction'], messageId: number }> = ({ payload, onAction, messageId }) => {
    ...
    return (
        <div>
            <h3 className="font-bold text-lg text-white">Configuration Wizard {isComplete ? '(Completed)' : level ? `(${isClone ? 'Clone' : 'New'} - Step ${step-1}/${totalSteps})` : ''}</h3>
            {step === 1 && (
                <div className="mt-2">
                    <p className="text-sm text-gray-300">What level of configuration do you want to create?</p>
            ...
            {step >= 2 && <p className="text-sm text-gray-400 mb-2">Level: <span className="font-semibold text-indigo-400">{level}</span></p>}
            
            {step === 2 && (
                <div className="mt-2">
                    <label htmlFor={`project-name-${messageId}`} className="block text-sm font-medium text-gray-300">Project Name</label>
                    <input type="text" id={`project-name-${messageId}`} value={projectName} onChange={(e) => setProjectName(e.target.value)} className="mt-1 block w-full bg-gray-900 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="e.g., Auto-billing" readOnly={isClone}/>
                </div>
            )}
             {step === 3 && level === 'Vendor' && (
                <div className="mt-2">
                    <p className="text-gray-300 mb-2">Project: <span className="font-semibold text-white">{data.projectName}</span></p>
                    <label htmlFor={`vendor-id-${messageId}`} className="block text-sm font-medium text-gray-300">Vendor ID</label>
                    <input type="text" id={`vendor-id-${messageId}`} value={vendorId} onChange={(e) => setVendorId(e.target.value)} className="mt-1 block w-full bg-gray-900 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="e.g., VEN-12345"/>
                </div>
            )}
            {finalStep && (
...
const TestStarterCard: React.FC<{ payload: any, onAction: CardRendererProps['onAction'], messageId: number, allBenchmarks: BenchmarkDataset[] }> = ({ payload, onAction, messageId, allBenchmarks }) => {
    ...
    return (
        <div>
            <h3 className="font-bold text-lg text-white">Start a New Test</h3>
            {payload.config && <p className="text-sm text-gray-400 -mt-1 mb-2">For Configuration: <span className="font-semibold text-indigo-400">{payload.config.projectName} {payload.config.vendorId || ''}</span></p>}
            ...
            <p className="text-gray-300 mt-4">Choose how you want to provide data for this test run.</p>
            
            <div className="mt-4 space-y-4">
                {/* Batch Test */}
                <div className="p-3 border border-gray-700 rounded-lg">
                    <label htmlFor={`batch-path-${messageId}`} className="block text-sm font-medium text-gray-300 flex items-center"><FolderIcon /><span className="ml-2">Run Batch Test</span></label>
                ...
                {/* Single File Test */}
                <div className="p-3 border border-gray-700 rounded-lg">
                     <h4 className="text-sm font-medium text-gray-300 flex items-center"><PaperclipIcon /><span className="ml-2">Run with Single File</span></h4>
                     <p className="text-xs text-gray-400 mt-1">Upload a single file for immediate testing.</p>
...
const TestResultsSummaryCard: React.FC<{ payload: any, onAction: CardRendererProps['onAction'], messageId: number }> = ({ payload, onAction, messageId }) => (
    <div>
        <h3 className="font-bold text-lg text-white">Test Results: {payload.project}</h3>

        {payload.benchmarkId && (
            <div className="mt-3 p-2 bg-gray-900/50 rounded-md border border-gray-700 flex justify-between items-center">
                <div>
                    <p className="text-xs text-gray-400">Verified Against Benchmark</p>
                    <p className="font-semibold text-teal-400">{payload.benchmarkId}</p>
                </div>
                <button 
        ...
        <div className="grid grid-cols-2 gap-4 mt-4 text-center">
            <div className="bg-gray-700/50 p-3 rounded-lg">
                <p className="text-2xl font-bold text-green-400">{payload.matched}</p>
                <p className="text-sm text-gray-300">Matched</p>
            </div>
             <div className="bg-gray-700/50 p-3 rounded-lg">
                <p className="text-2xl font-bold text-red-400">{payload.mismatched}</p>
                <p className="text-sm text-gray-300">Mismatched</p>
            </div>
        </div>
...
const InteractiveDiagnosticCard: React.FC<{ payload: any, onAction: CardRendererProps['onAction'], messageId: number }> = ({ payload, onAction, messageId }) => (
     <div>
        <h3 className="font-bold text-lg text-white">Interactive Diagnostic: {payload.recordId}</h3>
        <div className="mt-2 p-3 bg-gray-900 rounded-md font-mono text-xs text-gray-300 overflow-x-auto">
        ...
        {payload.status === 'resolved' ? (
             ...
             <div className="mt-4 flex items-center space-x-2 text-indigo-400">
                <LoadingSpinner />
...
const ConfirmationCard: React.FC<{ payload: any, onAction: CardRendererProps['onAction'], messageId: number }> = ({ payload, onAction, messageId }) => (
    <div>
        <h3 className="font-bold text-lg text-yellow-400 flex items-center"><ExclamationCircleIcon /> <span className="ml-2">Confirm Action</span></h3>
        <p className="text-gray-300 mt-2">
            You are about to <span className="font-bold text-red-400">{payload.action}</span> for project <span className="font-bold text-white">{payload.project}</span>.
            This is a production environment. Are you sure you want to proceed?
        </p>
        <div className="flex justify-end space-x-2 mt-4">
            <CardButton onClick={() => onAction(ActionType.CANCEL_ACTION, { messageId })} className="bg-gray-600 hover:bg-gray-700">Cancel</CardButton>
            <CardButton onClick={() => onAction(ActionType.CONFIRM_PAUSE_PRODUCTION, { messageId, project: payload.project })} className="bg-red-600 hover:bg-red-700">Confirm {payload.action}</CardButton>
        </div>
    </div>
);
...
const AlertCard: React.FC<{ payload: any }> = ({ payload }) => (
    <div>
        <h3 className="font-bold text-lg text-red-400 flex items-center"><ExclamationCircleIcon /> <span className="ml-2">Production Alert</span></h3>
        <p className="text-gray-300 mt-2">{payload.message}</p>
        <div className="mt-2 p-3 bg-gray-900 rounded-md font-mono text-xs text-gray-300">
...
const FileUploadCard: React.FC<{ payload: any, onAction: CardRendererProps['onAction'], messageId: number }> = ({ payload, onAction, messageId }) => {
    ...
    return (
        <div>
            <h3 className="font-bold text-lg text-white">Upload File</h3>
            {payload.status === 'idle' && (
                <>
                    <p className="text-gray-300 mt-1">Please upload the data file for processing.</p>
                    <input
                        type="file"
                        ref={fileInputRef}
...
const BenchmarkListCard: React.FC<{ payload: BenchmarkDataset, onAction: CardRendererProps['onAction'] }> = ({ payload, onAction }) => {
    ...
    return (
        <div>
            <h3 className="font-bold text-lg text-white flex items-center"><DatabaseIcon /><span className="ml-2">Golden Benchmark: {payload.projectName}</span></h3>
            <p className="text-sm text-gray-400 mt-1 italic">{payload.description}</p>
            
            <div className="mt-3 space-y-2 border border-gray-700 rounded-lg p-3 bg-gray-900/30 divide-y divide-gray-700">
                <div className="flex justify-between items-center py-1">
                    <p className="text-sm font-medium text-gray-400">Data Volume</p>
                    <p className="text-sm text-white font-semibold">{payload.dataVolume.toLocaleString()} Records</p>
                </div>
                 <div className="flex justify-between items-center py-1">
                    <p className="text-sm font-medium text-gray-400">Vendor Coverage</p>
                    <p className="text-sm text-white font-semibold">{payload.vendorCount} Vendors</p>
                </div>
                <div className="flex justify-between items-center py-1">
                    <p className="text-sm font-medium text-gray-400">Timeliness</p>
                    <p className="text-sm text-white font-semibold">{payload.timeliness}</p>
                </div>
            </div>

            <div className="mt-3">
                 <CardButton onClick={() => onAction(ActionType.VIEW_BENCHMARK_ON_METABASE, { benchmarkId: payload.id })} className="w-full bg-sky-600 hover:bg-sky-700">
                    View Full Dashboard on Metabase <ExternalLinkIcon />
                </CardButton>
            </div>

            <div className="mt-4">
                <h4 className="font-semibold text-gray-300 mb-2">Check Vendor Coverage</h4>
                <div className="flex items-center space-x-2">
                    <input
                        ...
                        className="flex-grow bg-gray-900 text-white placeholder-gray-400 border border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm"
                    />
                    <CardButton onClick={handleCheck} className="bg-gray-600 hover:bg-gray-700" disabled={!vendorQuery}>
                        Check
                    </CardButton>
                </div>
                {checkResult && (
                    <div className="mt-2 text-sm flex items-center">
                        {checkResult.covered ? (
                            <>
                                ...
                                <span className="text-green-300">Vendor <span className="font-bold">{checkResult.vendorId}</span> is covered.</span>
                            </>
                        ) : (
                             <>
                                <XCircleIcon className="h-5 w-5 text-red-400 mr-2" />
                                <span className="text-red-300">Vendor <span className="font-bold">{checkResult.vendorId}</span> is not covered.</span>
                            </>
                        )}
                    </div>
                )}
